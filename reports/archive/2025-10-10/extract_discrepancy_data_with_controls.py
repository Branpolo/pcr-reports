#!/usr/bin/env python3
"""
Extract discrepancy (QST) report data from database to JSON with control curves
Based on generate_qst_report_interactive_v2.py control fetching logic
"""
...
def get_controls_for_record(conn, run_id, target_name):
    """Get control curves for a specific run-target combination - from generate_qst_report_interactive_v2.py"""
    cursor = conn.cursor()
    
    # Check if qst_controls table exists
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='qst_controls'")
    ...
    try:
        ...
        # Table exists but maybe columns are different
        return []
    ...
    for row in cursor.fetchall():
        ...
        # Categorize control type (exact logic from generate_qst_report_interactive_v2.py)
        role_upper = role.upper() if role else ''
        if 'NC' in role_upper or 'NEGATIVE' in role_upper or 'BLANK' in role_upper:
            control_type = 'NC'  # Use NC/PC for consistency with other extractors
...
def get_other_observations(conn, disc_id):
    """Get other observation curves for the same well"""
    cursor = conn.cursor()
    
    # Check if qst_other_observations table exists
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='qst_other_observations'")
    if not cursor.fetchone():
        return []
...
def fetch_discrepancy_data(conn, limit=None):
    """Fetch QST discrepancy data with control curves"""
    cursor = conn.cursor()
...
def extract_well_curves_with_controls(conn, errors, limit=None):
    """Extract well curve data with control curves for each error"""
    print("\nFetching well curve data with controls...")
    well_curves = {}
    ...
    errors_to_process = errors[:limit] if limit else errors
    
    for i, error in enumerate(errors_to_process):
        if i % 500 == 0 and i > 0:
            print(f"  Processed {i}/{len(errors_to_process)} records...")
        ...
        if well_id not in processed_wells:
            ...
            if well_data:
                # For QST data, there's only one target per record
                target_data = well_data[0]
                target_name = target_data['target_name']
                mix_name = error['mix_name']
                run_id = error['run_id']
                
                # Get control curves for this run-target combination
                cache_key = f"{run_id}|{target_name}"
                if cache_key not in control_curves_cache:
                    control_curves_cache[cache_key] = get_controls_for_record(
                        conn, run_id, target_name
                    )
                
                # Get other observations for this well
                if well_id not in other_obs_cache:
                    other_obs_cache[well_id] = get_other_observations(conn, well_id)
                
                # Add control curves to target data
                target_data['control_curves'] = control_curves_cache[cache_key]
                target_data['other_observations'] = other_obs_cache[well_id]
                
                well_curves[str(well_id)] = {  # Use string key for consistency
                    'sample_name': error['sample_name'],
                    'mix_name': mix_name,
                    'run_id': run_id,
                    'targets': [target_data]  # List format for compatibility
                }
...
def main():
    parser = argparse.ArgumentParser(description='Extract discrepancy report data to JSON with control curves')
    parser.add_argument('--db', default='qst_discreps.db',
                       help='Path to QST database')
    parser.add_argument('--output', default='discrepancy_data_with_controls.json',
    ...
    if args.test:
        args.limit = 100
        args.output = 'discrepancy_data_with_controls_test.json'
    ...
    try:
        ...
        # Extract well curves with controls
        well_curves = extract_well_curves_with_controls(conn, errors, args.limit)
