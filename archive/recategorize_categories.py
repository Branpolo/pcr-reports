#!/usr/bin/env python3
"""
Database-agnostic script to categorize error/resolution/LIMS combinations.
Supports QST, Notts, and Vira databases with different LIMS status mappings.
"""
import csv
import json
import io
...
# Database-specific LIMS status normalization mappings
LIMS_MAPPINGS = {
    'qst': {
        # Detected variants
...
def normalize_lims_status(lims_status, db_type):
    """Normalize variant LIMS statuses to standard DETECTED/NOT DETECTED"""
    if not lims_status:
        return lims_status

    # Get database-specific mapping
    mapping = LIMS_MAPPINGS.get(db_type, {})
    return mapping.get(lims_status, lims_status)
...
def categorize_combination(well_type, error_code, resolution_codes, well_lims_status, db_type):
    """
    Categorization logic based on user feedback patterns.
    Database-agnostic with parameterized LIMS normalization.
    """

    lims = (well_lims_status or '').upper()
    lims_normalized = normalize_lims_status(lims, db_type)
...
def recategorize_csv(input_file, output_file, db_type):
    """Re-categorize CSV with updated logic"""

    rows = []
    with open(input_file, 'r', encoding='utf-8') as f:
        # Skip comment lines (including quoted ones)
        lines = []
        for line in f:
    ...
    if rows:
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            # Write header
            f.write(f'# {db_type.upper()} Category Mapping - v1 (Based on QST v3 logic)\n')
            f.write('#\n')
            ...
            f.write('#   Use same SOP_ categories (WELL_TYPE distinguishes)\n')
...
def main():
    parser = argparse.ArgumentParser(
        description='Categorize error/resolution/LIMS combinations for any database',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Categorize QST template
  python recategorize_categories.py --db-type qst \\
    --input output_data/qst_category_mapping_TEMPLATE.csv \\
    --output output_data/qst_category_mapping_v1.csv

  # Categorize Notts template
  python recategorize_categories.py --db-type notts \\
    --input output_data/notts_category_mapping_TEMPLATE.csv \\
    --output output_data/notts_category_mapping_v1.csv

  # Categorize Vira template
  python recategorize_categories.py --db-type vira \\
    --input output_data/vira_category_mapping_TEMPLATE.csv \\
    --output output_data/vira_category_mapping_v1.csv
        """
    )
    ...
    parser.add_argument('--input', required=True,
                       help='Input TEMPLATE CSV file')
    parser.add_argument('--output', required=True,
                       help='Output categorized CSV file')

    args = parser.parse_args()

    print(f"Database type: {args.db_type.upper()}")
    print(f"Input: {args.input}")
    print(f"Output: {args.output}")
    print()

    recategorize_csv(args.input, args.output, args.db_type)
